...
const DIFFICULTY_CONFIGS = {
  ...
  extreme: {
    ...
    multipliers: [
      ...
      893.22, 2235.72, 6096.15, 18960.33, 72432.75, 379632.82, 3608855.25,
    ],
  },
};
...
function Chicken() {
  ...
  const [betAmount, setBetAmount] = useState(4.0); // Bet amount state (min 4)
  const [isInputFocused, setIsInputFocused] = useState(false); // Track if input is focused
  const [inputValue, setInputValue] = useState("4.00"); // Local input value for editing
  ...
  useEffect(() => {
    const newLanes = generateLanesForDifficulty(
      DIFFICULTY_CONFIGS,
      currentDifficulty
    );
  ...
  const {
    ...
    error: userError,
    refetch,
  } = useGetUserInfo(token, tenantId); // Pass tenantId
  ...
  useEffect(() => {
    ...
    const currentValue =
      currentLaneIndex > 0 ? allLanes[currentLaneIndex - 1] : null;
    ...
    if (currentValue != null && currentValue === lastValue) {
      console.log(
        "Auto final jump to sidewalk from last multiplier value",
        currentValue
      );
  ...
  useEffect(() => {
    if (!isJumping) {
      ...
      let start = Math.min(
        currentLaneIndex,
        Math.max(0, totalLanes - windowSize)
      );
      let end = Math.min(totalLanes - 1, start + windowSize - 1);
  ...
  const incrementBet = () => {
    ...
    setBetAmount((prev) => parseFloat(Math.min(5000, prev + 4).toFixed(2)));
  };

  const decrementBet = () => {
    audioManager.playButtonClickAudio();
    setBetAmount((prev) => Math.max(4.0, parseFloat((prev - 4).toFixed(2))));
  };
  ...
  const handleBetInputBlur = () => {
    ...
    if (!isNaN(value)) {
      // Snap to nearest multiple of 4, clamp between 4 and 200
      const snapped = Math.round(value / 4) * 4;
      const clamped = Math.max(4.0, Math.min(5000, snapped));
      const finalValue = parseFloat(clamped.toFixed(2));
      setBetAmount(finalValue);
      setInputValue(finalValue.toFixed(2));
    } else {
      // Reset to current betAmount if invalid
  ...
  const handleCashOutWithToken = async () => {
    ...
    try {
      ...
      if (cashOutData) {
        ...
        if (
          typeof cashOutData?.winAmount === "number" &&
        ...
        try {
          ...
          console.warn(
            "User info refresh failed after cash out; continuing without reload.",
            e
          );
        }
  ...
  const startNewGame = async () => {
    ...
    if (userInfo.balance < betAmount) {
      setGameError(
        `Insufficient balance. You need ${betAmount.toFixed(2)} ETB to play`
      );
      return;
    }
    ...
    try {
      ...
      if (
        gameData?.canMoveFirstMove === false ||
        gameData?.isCrashOnFirstLane === true
      ) {
        // Animate to lane 1
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (
        ...
        !event.target.closest(".difficulty-selector")
      ) {
        setShowDifficultyDropdown(false);
      }
    };
  ...
  return (
    <div
      className="flex min-h-screen flex-col text-white overflow-hidden"
      style={{ backgroundColor: GAME_CONFIG.COLORS.BACKGROUND }}
    >
      ...
      <header
        className="px-1 flex items-center justify-between border-b"
        style={{
          backgroundColor: GAME_CONFIG.COLORS.BACKGROUND,
          borderColor: GAME_CONFIG.COLORS.TERTIARY_TEXT,
        }}
      >
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          <div
            className="px-3 py-1.5 rounded-lg flex items-center gap-1.5 sm:gap-2"
            style={{ backgroundColor: GAME_CONFIG.COLORS.ELEVATED }}
          >
            <FaCoins
              className="text-sm"
              style={{ color: GAME_CONFIG.COLORS.CASHOUT_BUTTON }}
            />
            {userInfo ? (
              <>
                <span
                  className="text-sm sm:text-md font-bold"
                  style={{ color: GAME_CONFIG.COLORS.BRIGHT_TEXT }}
                >
              ...
              <>
                <div
                  ...
                  style={{ backgroundColor: GAME_CONFIG.COLORS.TERTIARY_TEXT }}
                ></div>
                <span
                  className="text-xs"
                  style={{ color: GAME_CONFIG.COLORS.SECONDARY_TEXT }}
      ...
      <div
        ...
        className="grow relative  w-full h-[58%] overflow-hidden"
        style={{ backgroundColor: GAME_CONFIG.COLORS.ELEVATED }}
      >
        <TrafficProvider
          laneCount={assetsReady ? allLanes.length : 0}
          carSprites={[car1, car2, car3, car4, car5, car6]}
        >
          {assetsReady && (
            <Lane
              key={resetKey}
        ...
        {gameError && (
          <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-30">
            <div
              className="rounded-xl p-6 text-center shadow-2xl max-w-sm"
        ...
        <CashOutAnimation
          show={showCashOutAnimation}
          amount={lastCashOutAmount}
        />
      </div>
      ...
      <div className="p-2 sm:p-4">
        <div className="max-w-7xl mx-auto">
          <div
            className="rounded-2xl shadow-2xl p-2 sm:p-4 lg:p-6"
            style={{ backgroundColor: GAME_CONFIG.COLORS.ELEVATED }}
          >
            ...
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3 sm:gap-4 lg:gap-6">
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex flex-col gap-2 grow">
                  <div
                    ...
                    style={{
                      backgroundColor: GAME_CONFIG.COLORS.MORE_ELEVATED,
                    }}
                  >
                    <button
                      ...
                      className={`px-3 h-10 rounded-lg flex items-center justify-center text-white text-sm font-bold transition-all ${
                        currentLaneIndex > 0
                    ...
                    <div className="flex-1 text-center px-2">
                      <input
                        ...
                        disabled={currentLaneIndex > 0}
                        className={`text-white font-bold text-lg text-center w-full bg-transparent border-none outline-none ${
                          currentLaneIndex > 0
                            ? "opacity-50 cursor-not-allowed"
                            : isInputFocused
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        const balanceMax =
                          typeof userInfo?.balance === "number"
                            ? Math.floor(userInfo.balance * 100) / 100
                            : 5000;
                        const max = Math.min(5000, Math.max(4.0, balanceMax));
                        const maxValue = parseFloat(max.toFixed(2));
                        setBetAmount(maxValue);
                    ...
                    {[4, 8, 12, 20].map((amt, idx) => (
                      <button
                        ...
                        onClick={() => {
                          if (currentLaneIndex > 0) return;
                          setBetAmount(amt);
                          setInputValue(amt.toFixed(2));
                        }}
                        disabled={currentLaneIndex > 0}
                        className={`px-3 grow rounded-lg flex items-center gap-1 text-white text-sm font-semibold p-2 transition-all ${
                          currentLaneIndex > 0
                            ? "opacity-50 cursor-not-allowed"
                            : "hover:opacity-80 active:scale-95"
                        }`}
              ...
              <div
                ...
                style={{
                  opacity:
                    currentLaneIndex > 0 || isJumping || isCreatingGame
                      ? 0.6
                      : 1,
              ...
              >
                ...
                <div className="lg:hidden">
                  <button
                    ...
                    className={`w-full bg- flex items-center justify-between rounded-xl px-4 text-left ${
                      currentLaneIndex > 0 || isJumping || isCreatingGame
                  ...
                  >
                    <span
                      ...
                      style={{ color: GAME_CONFIG.COLORS.BRIGHT_TEXT }}
                    >
                      {DIFFICULTY_CONFIGS[currentDifficulty].name}
                    </span>
                    <svg
                      width="20"
                      height="20"
                      ...
                      style={{ fill: GAME_CONFIG.COLORS.BRIGHT_TEXT }}
                    >
                  ...
                  {showDifficultyDropdown &&
                    ...
                    !isCreatingGame && (
                      <div
                        className="absolute bottom-full left-0 right-0 mb-2 rounded-xl overflow-hidden z-50  shadow-2xl"
                        style={{ backgroundColor: GAME_CONFIG.COLORS.ELEVATED }}
                      >
                        {Object.entries(DIFFICULTY_CONFIGS).map(
                          ([key, config]) => (
                            <button
                              key={key}
                              onClick={() => {
                              ...
                              className={`w-full p-2 text-left  transition-colors ${
                                currentDifficulty === key ? "opacity-80" : ""
                              }`}
                              style={{
                                backgroundColor:
                                  currentDifficulty === key
                                    ? GAME_CONFIG.COLORS.ELEVATED
                                    : "transparent",
                              }}
                            >
                              <div
                  ...
                  <div className="flex gap-2">
                    {Object.entries(DIFFICULTY_CONFIGS).map(([key, config]) => (
                      <button
                        ...
                        className={`px-6 py-3 shadow-lg rounded-xl transition-all whitespace-nowrap font-medium text-base ${
                          currentLaneIndex > 0 || isJumping || isCreatingGame
                            ? "opacity-50 cursor-not-allowed"
                            : ""
                        }`}
                ...
                {currentLaneIndex === 0 ? (
                  ...
                  <div className="w-full lg:w-48">
                    <button
                      ...
                      className={`w-full h-[100px] font-bold px-8 rounded-xl text-xl transition-all duration-200 ${
                        !userInfo || userInfo.balance < betAmount
                          ? "opacity-50 cursor-not-allowed"
                      ...
                      style={{
                        backgroundColor:
                          ...
                          userInfo.balance < betAmount
                            ...
                            : GAME_CONFIG.COLORS.PLAY_BUTTON,
                        opacity:
                          !assetsReady || isCreatingGame || isJumping ? 0.7 : 1,
                        cursor:
                          !assetsReady || isCreatingGame || isJumping
                            ? "not-allowed"
                            : "pointer",
                      }}
                    >
                  ...
                  <div className="grid grid-cols-2 gap-3 w-full lg:w-64">
                    ...
                    <button
                      ...
                      style={{
                        ...
                        opacity:
                          isValidatingNext || isJumping || isCashingOut
                            ? 0.7
                            : 1,
                      }}
                    >
                      <div className="text-center">
                        ...
                        <div className="text-xl font-extrabold">
                          {(
                            betAmount * (allLanes[currentLaneIndex - 1] || 1)
                          ).toFixed(2)}{" "}
                          ETB
                        </div>
                      </div>
                    ...
                    <button
                      ...
                      disabled={
                        currentLaneIndex >= allLanes.length ||
                        isJumping ||
                        isDead ||
                        gameEnded
                      }
                      className={`w-full h-[100px] font-bold rounded-xl text-2xl transition-all duration-200 active:scale-95 text-white shadow-lg hover:opacity-90 ${
                        currentLaneIndex >= allLanes.length ||
                        isDead ||
                        gameEnded
                          ? "cursor-not-allowed"
      ...
      {showHowToPlay && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50">
          <div
            className="p-6 max-w-md rounded-2xl shadow-2xl"
          ...
          >
            ...
            <div className="space-y-2 text-sm text-gray-300">
              ...
              <p>
                3. The chicken will automatically move across multiplier zones
              </p>
